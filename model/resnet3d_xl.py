...
class ResNet(nn.Module):
    ...
    def forward_single(self, x):
        ...
        x = self.layer1(x)
        x = self.maxpool2(x) # deleted by Mr. Yan
        x = self.layer2(x)
...
def resnet50(extract_features, obtain_arc_id=2, **kwargs):
    """Constructs a ResNet-50 model.
    """
    use_temp_convs_set, temp_strides_set = obtain_arc(obtain_arc_id)
...
def Net(num_classes, extract_features=False, obtain_arc_id=2, loss_type='softmax', layer_num=50,
        weights=None, freeze_all_but_cls=False):
    net = globals()['resnet' + str(layer_num)](
        num_classes=num_classes,
        sample_size=50,
        sample_duration=32,
        extract_features=extract_features,
        obtain_arc_id = obtain_arc_id,
        loss_type=loss_type,
    )
